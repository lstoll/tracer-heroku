...
// New returns a new gRPC based server.StorageTransport
func New(srv *server.Server, listen string) server.StorageTransport {
	return &GRPC{
		srv:    srv,
		listen: listen,
	}
...
// NewWithGRPCServer returns a new srever.StorageTransport bound to the provided gRPC server
func NewWithGRPCServer(srv *server.Server, listen string, s *grpc.Server) server.StorageTransport {
	gt := &GRPC{
		srv:    srv,
		listen: listen,
	}
	pb.RegisterStorerServer(s, gt)
	return gt
}
